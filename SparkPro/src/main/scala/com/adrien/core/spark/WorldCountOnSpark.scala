...
object WorldCountOnSpark {
  def main(args: Array[String]): Unit = {
    //1、构建sparkConf对象 设置application名称
    val sparkConf = new SparkConf().setAppName("WordCountonSpark").setMaster("local[2]")
    ...
    */
    ...
    //3、读取数据文件
    val data: RDD[String] = sparkContext.textFile("/Users/luohaotian/Downloads/Jennifer/HelloApp/input/wordCount/wc.txt")
    //4、 切分每一行，获取所有单词
